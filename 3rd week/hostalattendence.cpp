...
int main() {
    ...
    int dayPresent[7] = {0};
     ...
     cout << "Input for 10 students and 7 days: " ;
    for (int i = 0; i < 10; i++) {
        for (int j = 0; j < 7; j++) {
            cin >> A[i][j];
        }
    ...
    int minAttendance = dayPresent[0];
    int minDay = 1;

    for (int j = 1; j < 7; j++) {
        if (dayPresent[j] < minAttendance) {
            minAttendance = dayPresent[j];
            minDay = j + 1;
        }
    }
    ...
    for (int i = 0; i < 10; i++) {
        cout << "Student " << i + 1 << ": " << studentPresent[i] << endl;
    }
